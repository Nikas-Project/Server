...
class API(object):
    ...
    def __init__(self, nikas, hasher):
        ...
        self.cache = nikas.cache
        self.signal = nikas.signal
        ...
        self.comments = nikas.db.comments
    ...
    @apiSuccessExample Success after the above request:
        {
            ...
            "text": "&lt;p&gt;Stop saying that! &lt;em&gt;nikas&lt;/em&gt;!&lt;/p&gt;",
            "dislikes": 0,
            "modified": null,
            "mode": 1,
            "hash": "e644f6ee43c0",
    ...
    def new(self, environ, request, uri):
        ...
        resp.headers.add("X-Set-Cookie", cookie("nikas-%i" % rv["id"]))
        return resp
    ...
    @apiGroup Comment
    @apiDescription
        Edit an existing comment. Editing a comment is only possible for a short period of time after it was created and only if the requestor has a valid cookie for it. See the [nikas server documentation](https://posativ.org/nikas/docs/configuration/server) for details. Editing a comment will set a new edit cookie in the response.
    @apiUse csrf
    ...
    def edit(self, environ, request, id):
        ...
        try:
            rv = self.nikas.unsign(request.cookies.get(str(id), ''))
            ...
            &lt;head&gt;
                &lt;script&gt;
                    if (confirm('Delete: Are you sure?')) {
                        ...
                        xhr.send(null);
                        xhr.onload = function() {
                            window.location.href = "https://example.com/example-thread/#nikas-13";
                        };
                    }
                &lt;/script&gt;
    ...
    def _process_fetched_list(self, fetched_list, plain=False):
        ...
        if plain:
            for item in fetched_list:
                item['text'] = self.nikas.render(item['text'])
    ...
    def feed(self, environ, request, uri):
        ...
        id.text = 'tag:{hostname},2018:/nikas/thread{uri}'.format(
        ...
        for comment in comments:
            ...
            id.text = 'tag:{hostname},2018:/nikas/{tid}/{id}'.format(
                hostname=hostname,
                tid=comment['tid'],
                id=comment['id'])
            ...
            content.text = self.nikas.render(comment['text'])

            if comment['parent']:
                ET.SubElement(entry, 'thr:in-reply-to', {
                    'ref': 'tag:{hostname},2018:/nikas/{tid}/{id}'.format(
                        hostname=hostname,
                        tid=comment['tid'],
                        id=comment['parent']),
                    'href': '{base}{uri}#nikas-{id}'.format(
    ...
    def preview(self, environment, request):
        ...
        return JSON({'text': self.nikas.render(data["text"])}, 200)
    ...
    def login(self, env, req):
        if not self.nikas.conf.getboolean("admin", "enabled"):
    ...
    def admin(self, env, req):
        ...
        if not self.nikas.conf.getboolean("admin", "enabled"):
            return render_template('disabled.html', nikas_host_script=nikas_host_script)
        try:
            data = self.nikas.unsign(req.cookies.get('admin-session', ''),
                                     max_age=60 * 60 * 24)
        except BadSignature:
            return render_template('login.html', nikas_host_script=nikas_host_script)
        ...
        for comment in list(comments):
            comment['hash'] = self.nikas.sign(comment['id'])
    ...
    def latest(self, environ, request):
        ...
        for comment in comments:
            ...
            processed['text'] = self.nikas.render(comment['text'])
